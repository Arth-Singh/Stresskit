...
def fig_checks(cards, out):
    ...
    ax.set_xlim(0, 50)
    ax.set_xlabel("graded reference cards (43 cards, 17 papers)")
    ax.set_title("Which checks the 43 graded cards pass (undecided: the 95% CI straddles the bar)", fontsize=10, loc="left", color=INK)
    style(ax)
    fig.tight_layout()
