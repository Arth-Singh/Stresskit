...
.nav { margin-bottom: .8rem; font-size: .9rem; }
.trace { margin-top: 1.8rem; border-top: 1px solid #eaeef2; padding-top: 1.2rem; }
.trace h2 { font-size: 1rem; margin: 0 0 .6rem; }
...
def card_html(d: Dict[str, Any], extra_sections: Optional[List[str]] = None,
              nav_html: str = "") -> str:
    """Render any StressKit artifact dict as a standalone HTML page.

    ``extra_sections`` are trusted HTML fragments (e.g. an embedded trace
    chart) inserted after the checks table; ``nav_html`` goes above the
    card (used by the site generator for a back link).
    """
...
<body>
<div class="wrap">{nav_html}<div class="card">
<div class="head">
