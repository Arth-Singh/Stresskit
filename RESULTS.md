...
Ordered by how recent and how safety-relevant the audited paper is, not by
audit date. Seventeen of the 21 papers are July or August 2026 preprints
(arXiv 2607/2608), and most cards run on Qwen3.5/3.6, Qwen2.5, Gemma-3,
Gemma-4 or Llama-3.1/3.2. Eight of the 49 cards run on GPT-2 alone: four
because the audited 2026 paper is a GPT-2 paper — reproducing a released
number requires the model it was released on — and four (IOI on three sizes,
greater-than) because they are instrument validation, in their own table
below rather than in the audit leaderboard.


| paper | model(s) | grade | conf. | checks | runs | reproduced the released number? | result in one line | audited | card |
|---|---|---|---|---|---|---|---|---|---|
...
### Instrument validation (not audit targets)

Two classic circuit-finding tasks, run to check that the battery behaves on
findings whose structure is already known and agreed. They are the only
GPT-2-only entries chosen by us rather than inherited from the audited paper,
and they carry the six-run instability evidence in
[What a paper-sized run count would have reported](#what-a-paper-sized-run-count-would-have-reported).
...
size (`references/figs/verdict_settle_n.png`). Thirty cards carry a trace;
the 18 whose real and null runs are recoverable were regenerated under rule
v0.4 on 2026-09-03, and the other 8 keep the v0.3 trace they were committed
with, because a hash-only run record or a digest-only direction vector cannot
be regraded from the card. Each regenerated trace reproduces its card's grade
and its settle point exactly.

At six runs, the number of seeds a paper typically reports, the modal grade of
the subsets matches the full battery on 23 of 30 cards, but the grade a single
six-run study would report is wrong at least a quarter of the time on 16 of the
30 (sycophancy on Gemma-3-12B 87%, REINS-Gate 73%, homonym on GPT-2 70%, IOI on
GPT-2 large 63%, HARC on Llama 53%, refusal on Qwen3.5-9B 53%, SAE inertness
53%, AMS 50%, Mechanistic Tomography 50%, CoT-faithfulness steering 47%, CIF
43%, HARC on Qwen 40%), and only 11 of 30 verdicts have settled by then. No
verdict fails to settle by its full run count.

Seven cards would report a different modal grade at six runs than at full size,
and the error runs in both directions: too generous on AMS (C for D), homonym
on GPT-2 (B for C), refusal on Qwen3.5-9B (B for C) and REINS-Gate (A for B);
too harsh on HARC-Llama (C for B), sycophancy on Gemma (C for B) and IOI large
(C for B). The numbers come from each card's `.trace.json` (`per_size["6"]`).
